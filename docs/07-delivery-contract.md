...
9. The completed attempt appends immutable evidence and a next-review proposal from the configured scheduler (`fsrs` by default, `sm2` or `module:<module-id>` optional) that names the algorithm, its version, configuration, learning context, source event, and due time. Trusted scheduler modules require opt-in on each writing run. Their successful receipt is atomic with the attempt and proposal; every rejected execution writes nothing. Changing the algorithm on a workspace with recorded state fails closed until `scheduler switch` records the change.
...
11. The extension boundary validates and invokes one trusted local executable through `virtuoso/module@0.1` only when calling code passes `allow_trusted=True`. Tests prove that unknown fields/schema, missing calling-code opt-in, shell or command-wrapper indirection, malformed or incomplete typed output, timeout, oversized output, nested private-state projections, descendant spawning, and manifest identity drift fail closed. A scheduler module receives only the typed `scheduler.request` projection in the existing request envelope and must return `proposed_state`. There is no public generic CLI command for module execution and no consent dialog. Descendant spawning is denied with a POSIX process limit and execution fails closed when that control is unavailable. The trusted executable still has the invoking user's file permissions, so this boundary is not an OS sandbox.
