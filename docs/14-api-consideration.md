...
**B. Formal Python SDK.** Publish the services as the supported library interface with versioned schemas. They already exist, so this is cheap, but it serves only Python consumers, and every other harness still needs the CLI.
