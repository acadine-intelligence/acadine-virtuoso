...
class DelayedTransferCheckMigrationTests(unittest.TestCase):
    ...
    def test_v4_workspace_migrates_to_v5_without_changing_existing_transfer_evidence(
        self,
    ) -> None:
    ...
    def test_v5_open_rejects_missing_transfer_check_table_instead_of_repairing(
        self,
    ) -> None:
        service, _event = self._workspace_with_transfer()
        with sqlite3.connect(service.db_path) as db:
...
class DueTransferCheckAndPredictionTests(unittest.TestCase):
    ...
    def test_due_list_fails_closed_on_malformed_or_mismatched_stored_timestamp(
        ...
        with sqlite3.connect(self.service.db_path) as db:
            db.execute(
                "UPDATE transfer_checks SET due_at = 'not-a-timestamp' WHERE check_id = ?",
                (self.check.check_id,),
            )
        ...
        with sqlite3.connect(self.service.db_path) as db:
            db.execute(
                ...
                (
                    (self.due_at + timedelta(seconds=1)).isoformat(),
                    self.check.check_id,
                ),
            )
